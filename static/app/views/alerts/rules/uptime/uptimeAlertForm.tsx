...
import {
  createEmptyAssertionRoot,
  UptimeAssertionsField,
} from "./assertions/field";
...
function UptimeAlertFormContent({ handleDelete, rule }: Props) {
  ...
  const { hasRuntimeAssertions, hasAiAssertionSuggestions } =
    useUptimeAssertionFeatures();
  ...
  const [newEnvironment, setNewEnvironment] = useState<string | undefined>(
    undefined
  );
    ...
    <Form
      ...
      onPreSubmit={() => {
        ...
        if (!hasRuntimeAssertions) {
          const assertion = formModel.getValue<UptimeAssertion | null>(
            "assertion"
          );
      ...
      <List symbol="colored-numeric">
        ...
        <FormRow>
          <SentryProjectSelectorField
            ...
            disabledReason={t(
              "Existing uptime rules cannot be moved between projects"
            )}
            name="projectSlug"
            label={t("Project")}
        ...
        <Configuration>
          <ConfigurationPanel>
                ...
                tct(
                  "The amount of time between each uptime check request. Selecting a period of [interval] means it will take at least [expectedFailureInterval] until you are notified of a failure. [link:Learn more].",
                  {
                    ...
                    expectedFailureInterval: (
                      <strong>
                        {getDuration(
                          Number(model.getValue("intervalSeconds")) * 3
                        )}
                      </strong>
                    ),
                  }
            ...
            <RangeField
              ...
              step={250}
              tickValues={[
                1_000, 10_000, 20_000, 30_000, 40_000, 50_000, 60_000,
              ]}
          ...
          <Observer>
            {() => (
              <HTTPSnippet
                ...
                body={
                  methodHasBody(formModel) ? formModel.getValue("body") : null
                }
            ...
            <NumberField
              ...
              help={({ model }) => {
                const intervalSeconds = Number(
                  model.getValue("intervalSeconds")
                );
                const threshold =
